...
for epoch in range(config.max_epochs):
    ...
    if evaluation_result[config.val_metric] > best_metric:
        ...
        model_name = f"models/sasrec-inbatch-unique-logq-old-{config.dataset_name}-step:{step}-t:{config.gbce_t}-negs:{config.negs_per_pos}-emb:{config.embedding_dim}-dropout:{config.dropout_rate}-metric:{best_metric}.pt" 
