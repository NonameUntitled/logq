...
for epoch in range(config.max_epochs):
    ...
    for batch_idx in pbar:
        ...
        negative_correction = torch.clamp(item_interactions_num[mixed_negatives_ids] / (total_interraction_sum - item_interactions_num[labels][..., None]), min=1e-10)
    ...
    if evaluation_result[config.val_metric] > best_metric:
        ...
        model_name = f"models/sasrec-mns-logq-new-no-fix-{config.dataset_name}-step:{step}-t:{config.gbce_t}-negs:{config.negs_per_pos}-emb:{config.embedding_dim}-dropout:{config.dropout_rate}-metric:{best_metric}.pt" 
