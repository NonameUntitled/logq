...
for epoch in range(config.max_epochs):
    ...
    for batch_idx in pbar:
        ...
        negative_mask = labels[:, None] != negatives
    ...
    if evaluation_result[config.val_metric] > best_metric:
        ...
        model_name = f"models/sasrec-uniform-{config.dataset_name}-step:{step}-t:{config.gbce_t}-negs:{config.negs_per_pos}-emb:{config.embedding_dim}-dropout:{config.dropout_rate}-metric:{best_metric}.pt" 
