...
for epoch in range(config.max_epochs):
    ...
    for batch_idx in pbar:
        ...
        scores = torch.einsum('se, ne -> sn', last_hidden_state[mask], output_embeddings.weight)
        loss = torch.nn.functional.cross_entropy(scores, labels[mask])
        loss.backward()
        optimiser.step()
        optimiser.zero_grad()
        loss_sum += loss.item()
    ...
    if evaluation_result[config.val_metric] > best_metric:
        ...
        model_name = f"models/sasrec-full-{config.dataset_name}-step:{step}-t:{config.gbce_t}-negs:{config.negs_per_pos}-emb:{config.embedding_dim}-dropout:{config.dropout_rate}-metric:{best_metric}.pt" 
